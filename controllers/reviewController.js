...
function buildTree(rows) {
  const byId = {};
  rows.forEach(r => { byId[r.id] = Object.assign({}, r, { replies: [] }); });
  const roots = [];
  rows.forEach(r => {
...
exports.postReview = async (req, res) => {
  try {
    ...
    if (!clientId) return res.status(403).json({ ok: false, error: 'Not authenticated' });
    ...
    if (isNaN(r) || r < 1 || r > 5) return res.status(400).json({ ok: false, error: 'rating must be 1-5' });

    // optional: verify order belongs to client and refers to vendor (if orderId provided)
    if (orderId) {
      const order = await orderModel.findById(orderId);
      if (!order || order.client_id !== clientId || order.vendor_id !== vendorId) {
...
exports.postReplyByClient = async (req, res) => {
  try {
    ...
    if (!comment) return res.status(400).json({ ok: false, error: 'comment required' });

    // Could validate parent exists and belongs to vendor etc.
    // For simplicity: create reply with same vendor id as parent
    const parentRows = await reviewModel.getReviewsByVendor(/* hack: we'll fetch parent via DB */);
