...
// Example:
//   node createSuperAdmin.js super@admin.local 'Ellaberry1@' 'Super Admin'
//
...
function makePool() {
  ...
  const wantsSSL = (process.env.PGSSLMODE && process.env.PGSSLMODE.toLowerCase() === 'require')
...
async function main() {
  ...
  if (!password) {
    ...
    console.error('Example: node createSuperAdmin.js super@admin.local "Ellaberry1@" "Super Admin"');
    process.exit(2);
  }
  ...
  if (!pool) {
    ...
    console.error('Make sure .env has DATABASE_URL or PGHOST/PGUSER/PGPASSWORD/PGDATABASE set.');
    process.exit(3);
  }
  ...
  try {
    ...
    if (err && /password/i.test(err.message || '')) {
      console.error('Hint: check DATABASE_URL / PGPASSWORD in your .env and ensure password is a string.');
    }
    if (err && /SSL/i.test(err.message || '')) {
      console.error('Hint: if your DB requires SSL, ensure PGSSLMODE=require in .env (already set in your .env).');
    }
