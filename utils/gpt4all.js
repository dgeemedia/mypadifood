...
function scoreDocs(query, docs, topN = 3) {
  ...
  return scores.filter(s => s.score > 0).slice(0, topN).map(s => s.doc);
}
...
function runCli(prompt) {
  return new Promise((resolve, reject) => {
    ...
    execFile(CLI_PATH, args, { timeout: MAX_RESPONSE_MS, maxBuffer: 1024 * 1024 * 8 }, (err, stdout, stderr) => {
      if (err) {
        return reject(new Error(`CLI error: ${err.message} ${stderr ? '| stderr: ' + stderr : ''}`));
      }
