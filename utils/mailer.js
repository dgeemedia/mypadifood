...
const mailFrom = process.env.MAIL_FROM || `no-reply@${process.env.APP_DOMAIN || 'mypadifood.local'}`;
...
function createSmtpTransporterFromEnv() {
  ...
  transporter.verify()
    .then(() => { smtpVerified = true; console.log('SMTP transporter verified.'); })
    .catch(err => {
      smtpVerified = false;
...
async function sendMail({ to, subject, text = '', html = '' }) {
  ...
  if ((MAIL_SEND_METHOD === 'sendgrid' || !MAIL_SEND_METHOD) && SENDGRID_API_KEY) {
    try {
  ...
  console.warn('No working mail transport available — logging email to console (fallback).');
