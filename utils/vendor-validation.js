...
const registrationRules = () => {
  return [
    body('name')
      .trim()
      .notEmpty().withMessage('Vendor / Stall name is required.')
      .isLength({ min: 2 }).withMessage('Vendor name must be at least 2 characters.'),
    ...
    body('address')
      ...
      .isLength({ min: 5 }).withMessage('Address looks too short.'),

    body('phone')
      .trim()
      .notEmpty().withMessage('Phone is required.')
      .isLength({ min: 6 }).withMessage('Phone looks too short.'),
    ...
    body('food_item')
      .trim()
      .notEmpty().withMessage('Please list at least one food item.'),
...
const checkRegData = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    ...
    const {
      name, state, lga, address, phone, email, food_item, base_price, latitude, longitude, location_source
    } = req.body;
