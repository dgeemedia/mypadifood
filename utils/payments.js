...
async function initPaystack({ email, amount }, orderId = null) {
  ...
  if (!resp.ok || !json || !json.status) {
    const errMsg =
      (json && (json.message || JSON.stringify(json))) || `Paystack init failed (${resp.status})`;
...
async function verifyPaystack(reference) {
  ...
  return {
    success: json.data.status === "success" || json.data.status === "successful",
    data: json.data,
    raw: json,
  };
}
...
async function verifyFlutterwave(transaction_id) {
  ...
  const success =
    json.data && (json.data.status === "successful" || json.data.status === "success");
