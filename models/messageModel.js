...
async function getMessagesByOrder(orderId, limit = 200) {
  ...
  rows.forEach((msg) => {
    ...
    if (msg.sender_type === 'client') {
      ...
      msg.display_name =
        msg.admin_name || (msg.sender_id ? `Admin ${msg.sender_id}` : 'Admin');
...
async function markReadByAdmin(orderId) {
  await pool.query(
    'UPDATE order_messages SET read_by_admin = TRUE WHERE order_id = $1',
    [orderId]
  );
}
...
async function markReadByClient(orderId) {
  await pool.query(
    'UPDATE order_messages SET read_by_client = TRUE WHERE order_id = $1',
    [orderId]
  );
}
