...
(function () {
  ...
  function ensureSocket() {
    ...
    socket.on("order_message", (payload) => {
      ...
      try {
        ...
        const msgOrderId =
          payload &&
          (payload.orderId || payload.order_id || (payload.message && payload.message.order_id));
  ...
  function handleIncomingOrder(order) {
    ...
    const clientAddress =
      order.client_address || order.clientAddress || order.client_address || "—";
  ...
  async function loadMessagesFor(orderId) {
    ...
    try {
      ...
      const isBotPrompt = (json.messages || []).some((x) => {
        ...
        return (
          (x.sender_type === "bot" || (x.senderType || "").toLowerCase() === "bot") &&
  ...
  function appendMessage(m) {
    ...
    wrapper.className =
      "msg " + (senderType === "client" ? "client" : senderType === "admin" ? "admin" : "bot");
  ...
  try {
    ...
    if (raw) {
      ...
      if (pos && typeof pos.left === "number" && typeof pos.top === "number") {
        ...
        modal.style.left =
          Math.max(8, Math.min(window.innerWidth - (modal.offsetWidth + 8), pos.left)) + "px";
        modal.style.top =
          Math.max(8, Math.min(window.innerHeight - (modal.offsetHeight + 8), pos.top)) + "px";
