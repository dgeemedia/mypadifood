...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!e.target.closest('.mobile-drawer') && !e.target.closest('#mobile-menu-toggle')) closeDrawer();
  });
  ...
  async function fillLocation(event, context) {
    ...
    try {
      ...
      if (!('geolocation' in navigator)) { alert('Geolocation not supported by this browser. Please fill location manually.'); return; }
      ...
      try {
        ...
        if (stateField && foundState) {
          if (stateField.tagName === 'SELECT') {
            ...
            if (opt) stateField.value = opt.value; else { const tmp = document.createElement('option'); tmp.value = foundState; tmp.text = foundState; stateField.appendChild(tmp); stateField.value = foundState; }
        ...
        if (lgaField && foundLga) {
          if (lgaField.tagName === 'SELECT') {
            ...
            if (opt2) lgaField.value = opt2.value; else { const tmp2 = document.createElement('option'); tmp2.value = foundLga; tmp2.text = foundLga; lgaField.appendChild(tmp2); lgaField.value = foundLga; }
      ...
      if (btn) { btn.disabled = false; btn.textContent = originalText || 'Use my device location'; }
    }
  }
  ...
  document.addEventListener('click', function (ev) {
    ...
    if (!btn) return;
    ev.preventDefault();
    const row = btn.closest('label') || btn.closest('.form-row') || btn.closest('form');
    let pwd;
    if (row) pwd = row.querySelector('input[type="password"], input[type="text"][name*="password"], input[type="password"][name*="password"]');
    if (!pwd) return;
  ...
  (function () {
    ...
    document.getElementById('dev-verification-close').addEventListener('click', dismiss);
    document.getElementById('dev-verification-ok').addEventListener('click', dismiss);
    document.getElementById('dev-verification-copy').addEventListener('click', function () {
      const text = link;
