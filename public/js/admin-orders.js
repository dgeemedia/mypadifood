...
(function () {
  ...
  document.addEventListener('click', async (ev) => {
    ...
    if (t.closest && t.closest('.admin-notif')) {
      ...
      const orderId =
        wrapper &&
        (wrapper.dataset.orderId || wrapper.getAttribute('data-order-id'));
      if (orderId) {
        ...
        if (!notifications || notifications.length === 0) {
          ...
          notifPanel.innerHTML = notifications
            .map((n) => {
              ...
              const orderId =
                n.order_id ||
                (n.payload && (n.payload.order_id || n.payload.orderId)) ||
                '';
              const payloadText =
                (n.payload &&
                  (n.payload.order_summary ||
                    n.payload.item ||
  ...
  socket.on('order_message', (payload) => {
    try {
      const orderId =
        payload && (payload.orderId || payload.order_id || payload.order);
