...
(function () {
  ...
  socket.on('connect', () => {
    try { socket.emit('admin_join'); } catch (e) { /* ignore */ }
  });
  ...
  function renderOrderCard(o, opts = {}) {
    ...
    const statusBadge = opts.completed ? `<span class="completed-badge" style="background:#f1f5f9;color:#111;padding:4px 8px;border-radius:8px;font-weight:700;">Completed</span>` : '';
    ...
    try {
      ...
      if (notifPanel) {
        if (!notifications || notifications.length === 0) {
          notifPanel.innerHTML = '<div class="admin-notif-empty" style="color:var(--muted)">No notifications</div>';
        } else {
          notifPanel.innerHTML = notifications.map(n => {
            ...
            const payloadText = (n.payload && (n.payload.order_summary || n.payload.item || JSON.stringify(n.payload))) || '';
  ...
  socket.on('new_order', data => {
    try {
      ...
      // optional notif
      if (window.Notification && Notification.permission === 'granted') new Notification('New order', { body: `${data.client_name || 'Client'}` });
    } catch (e) {
  ...
  socket.on('order_message', payload => {
    try {
      ...
      if (!card) {
        ...
        if (container) container.insertAdjacentHTML('afterbegin', renderOrderCard({ id: orderId, client_name: (payload.client_name || payload.clientName || '') }));
      }
      // visually highlight
  ...
  socket.on('order_completed', ({ orderId }) => {
    try {
      ...
      if (card && completedContainer) {
        ...
        card.querySelector('.order-meta').insertAdjacentHTML('beforeend', ` <span class="completed-badge">Completed</span>`);
        ...
        fetch('/admin/orders/completed?format=json').then(r => r.json()).then(json => {
          if (json && json.orders) {
            completedContainer.innerHTML = json.orders.map(o => renderOrderCard(o, { completed: true })).join('');
          }
