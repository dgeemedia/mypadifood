    ...
    try {
      ...
      if (!quoteTrim || charCount < 8) {
        pushMessage(
          req,
          'error',
          'Please provide a testimonial (at least 8 characters).'
        );
      ...
      if (!consentGiven) {
        pushMessage(
          req,
          'error',
          'Please confirm you consent to publish this testimonial when approved.'
        );
        return res.redirect(req.get('Referrer') || '/client/dashboard');
      }
      ...
      pushMessage(
        req,
        'success',
        'Thank you! Your testimonial will be reviewed before publishing.'
      );
...
router.get('/submit', auth.requireClient, (req, res) => {
  res.render('testimonials/submit', {
